...
# "This is me" no longer links on submit — it files a claim for review
# (22 Jul 2026). The behaviour those tests used to pin now lives in
# tests/test_author_claims.py, which covers both paths and the approval step.
...
async def test_create_author_is_me_does_not_relink_existing_author(client):
    ...
    # Re-"adding" the same name (get-or-create hit) must not touch the existing
    # row's link — with claims, neither call links anything in the first place.
