...
**Shipping state: LIVE ON GOOGLE PLAY, 31 Aug 2026 — iOS still in App Store
Review.** Android is public at
<https://play.google.com/store/apps/details?id=in.kitabi.kitabi> (package
`in.kitabi.kitabi`); iOS build **136** (0.1.0) is still with App Store Review. Both
were cut from the `main` that carries the device-safe migration rework (`71a3b2e` —
the v12 rebuild would have wedged every install upgrading from ≤v11; do not ship
anything older). **The Play URL is now a fact the site depends on** — it is the
single constant `PLAY_STORE_URL` in `landing-page/functions/_lib/components.js`, and
the App Store half of that file is a deliberate `null` that renders an inert "soon"
badge. When iOS is approved, filling in `APP_STORE_URL` turns that badge into a link
on its own; `landing-page/app.html` carries the same pair by hand (hero CTA, closing
band, FAQ, and the `installUrl`/`downloadUrl` in its JSON-LD). Complete listings live in `docs/store/{android,ios}/`: copy, keywords,
