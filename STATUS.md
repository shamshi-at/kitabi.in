...
- **12 Sep 2026** — **The reading timer can throw a sitting away.** Owner request: a reader starts the clock, then can't read, and doesn't want the time filed. Until now the only exit was Stop & log — file a sitting that never happened, then find the book and delete the row from its reading log. **Discard this sitting** now sits below Stop & log on the watch face, in plain grey rather than red, and opens a confirmation that names the elapsed time being thrown away and — when the sitting holds any — promises the notes are kept, because a thought written down is not part of the timing. **The mini-bar carries the same door**, as a muted ✕ beside its gold stop: that bar is what a reader who left a timer running meets when they reopen the app, before they meet any screen. Both go through one `discardSessionFlow` (`features/library/discard_session_flow.dart`) — the question, the discard and the snackbar — with only the aftermath owned per surface, because a confirmation asked in two files is a confirmation that starts answering differently. The mini-bar is also the hard caller: it is built *only* while a sitting is live, so its widget and its `ref` are gone the moment the discard lands, and every handle is captured before the first await (probed — reading the snackbar's messenger off the caller's own context after the await makes it silently never appear, which is the 19 Jul / 31 Jul 2026 report against this exact widget). `discardActiveSession` is its own function, not a `log: false` flag on `stopAndLogActiveSession`: "is this reading?" is the one thing the two paths answer differently, and it does not belong behind a boolean halfway down a function whose name promises a row. Everything they agree on moved into a shared `_closeSitting` — the seven `active_session_*` rows, the check-in notification, the enforcement task, the lock-screen clock — so the two endings cannot drift, which every pair of stop surfaces in this app has done at least once. Three things the discard has to get right that a stop does not. The account still believes a timer is running, so it leaves the same pending-stop note a stop does and `publishStop` retries it until the DELETE lands; the key's meaning widened from "stopped and logged" to "ended here", and all three of its readers only ever ask "does the server still hold a sitting we have finished with?". A note written mid-sitting names a `reading_sessions` id that is now never coming, so the notes are cut loose (`session_id` cleared, pending link forgotten) rather than left pointing at a sitting no query can resolve — without that the link would be re-checked on every sync drain forever. And the screen must leave exactly **once**: the discard clears the session, which `build`'s own "stopped on another device" guard reads as its cue to leave, so an unguarded discard exits twice and pops the book page out from under the reader — `_leaving` is set synchronously before the first await. Adding the button also exposed an older layout bug it did not cause: the watch face centred a fixed ~430px slab (cover, 220px dial, zone badge) with a bare `Center`, so a short screen got an overflow bar across the dial. It reads the box it is given now (`LayoutBuilder` + a scroll fallback), the same rule the share cards learned on 8 Sep. Not added to Home's currently-reading card or the book page's reading card, which still offer Stop & log alone — both are tight rows of oxblood controls (Home's has its own overflow test) and a third glyph there is a design call, not a wiring one. Tests: `reading_discard_test.dart` (nothing logged, nothing queued, every surface down, the account note left, notes survive detached, only *this* sitting's links forgotten, a discard with nothing running still clears the clock), `reading_timer_discard_screen_test.dart` (asks first; "Keep timing" keeps the clock running; one tap, one exit — probed by disabling the guard and watching it land a page too far back; and the face survives a 320x500 screen) and `mini_bar_discard_test.dart`, which drives the real `ShellScaffold`.
- **11 Sep 2026** — **The catalogue seed becomes a nightly job, and the gate is that a record is complete or it is not created.** Plan: [docs/catalog-intake-plan.md](docs/catalog-intake-plan.md). Until now the catalogue grew two ways — readers adding books, and a human running the `etl/` scripts against a 9 GB dump — and the dump path imported records that then needed repairing: `09_marc_cleanup.py` and `10_title_restore.py` (35 KB between them) exist for no other reason, and `10` needs an LLM *and a human* per book. **The fix is the source, not a fifth cleanup pass.** Measured first: OpenLibrary is 98.3% complete for the Indian-English *head* and hopeless for Indic-language records (they are Library of Congress MARC); Mathrubhumi Books' own storefront is 87.5% valid-ISBN, 100% author, 100% cover, 50% **back** cover; LookaBook has the publisher breadth (DC Books 550 titles) and carries an ISBN on 15.6% of pages, so it cannot fill Mathrubhumi's gaps. **No paid API is being bought** — ISBNdb ($14.99–$299.99/mo, billed per *result*) would be paying for the half that already works. **P1, the spine, shipped:** `catalog_intake` (migration `000053`, RLS on with zero policies) is a staging table discovery writes to and *only* to, so a source can be crawled and re-crawled with no catalogue row appearing; `services/intake_gate` is pure and 53-test-covered, requiring title + author + publisher + cover + a **checksum-valid ISBN**, cleaning MARC punctuation at the door via `marc_cleanup`, and separating **missing** (another source may fill it) from **fatal** (a supplied heading is not a book); `services/intake_service` promotes through `catalog_service.create_work_with_edition`, so transliteration columns, slugs, the ISBN conflict guard and `merge_service.canonical` publisher resolution are all inherited rather than backfilled the way `etl/04_load.sql` needs `06_backfill_script.py`; and every promotion leaves its receipt on the candidate, so `revert` is one query. `jobs/catalog_intake` runs at 02:30 UTC under an advisory lock on a **cron, not an interval** — this service redeploys on every push to main, and a daily budget that resets whenever someone ships is not a budget. **Dormant unless `CATALOG_INTAKE_ENABLED=1`**, so merging it creates nothing; production will opt in via the Dockerfile, in the repo rather than a dashboard, like `INDEXNOW_ENABLED`. Verified end-to-end against live OpenLibrary into the dev database: 47 discovered, **0 catalogue rows written by discovery**, 10 promoted with valid ISBNs, real publishers and slugs. That live run found three things mocks could not — the work-level cover fallback was dead code because `cover_i` was never requested; `publisher:"Juggernaut Books"` matches an *Australian* press and shelved a YA novel under an Indian-English seed, so a publisher seed now requires the house to actually be named; and OL returns names like `Juggernaut Books Pty,`, where a trailing comma is never load-bearing even though an internal one usually is. A fourth was in shared code: `marc_cleanup.clean_work` needed **two** passes on `"Mukajjiya kanasugaḷu" /`, because stripping the dangling ` /` re-exposes quotes `_unquote` has already gone past — exactly the "a cleanup that only settles on the second run is one nobody can tell is finished" trap the etl README names. It iterates to a fixed point now, which is what a door-time gate depends on. It also corrected a planning number: the 98.3% was measured on reading-log-ranked results, and a publisher's whole backlist is nearer **72%** complete, the entire gap being cover art — which is P2. Tests: `test_intake_gate.py`, `test_intake_service.py`, `test_intake_openlibrary.py`.
- **8 Sep 2026** — **Share cards: the caption ships with the image, the month card shows the month, and a card can now carry a link.** Five things from one owner report against Apple Books' reading card. **(1) The caption arrives.** `captureAndShareCard` hands the OS the PNG *and* the text again. It had been sharing the image alone since 26 Aug on the conclusion that WhatsApp drops one when handed both — but `git show efd5d6b^` shows the readiness check was already `RenderObject.debugNeedsPaint`, whose value is assigned only inside an `assert`, so every **release** build threw and every share on every phone took the text-only *fallback*. That was fixed 6 Sep; the workaround built on the misdiagnosis outlived it by two days, and the owner reported the clipboard step itself as the defect. The clipboard copy stays, silent, as belt-and-braces; the two l10n strings that described the workaround are gone. **(2) The month card draws the whole month.** `CardHeat` used to crop trailing all-future weeks, so a card sent on the 8th stopped mid-week on the 12th and read as a picture that had been cut off; unread past days were also `paperDeep` on `paper`, 7/255 apart, so the grid behind the gold days was invisible. Fixing it exposed an older bug: the calendar sized its cells from the width it *asked* for, and on the Square — where it shares a row with the hero and gets about 60px of the 96 it wants — the month wrapped into two columns off the bottom of the card. It now reads the box it is given (`LayoutBuilder`, honouring `constraints.maxHeight` too, so a viz inside an `Expanded` shrinks rather than pushing the wordmark off) and lays weeks out as explicit `Row`s of seven. **(3) A door to the daily card** on the two surfaces that end a sitting — the timer's wax-seal face and the quick-stop sheet — through one shared `shareTodaysReading`. The Today card itself is unchanged and has existed since 26 Aug; it had one door, in Insights, three taps from the moment a reader actually stops reading. **(4) A month switcher on Insights**: the Month chip gains the same body-tap/arrow sub-choice the Year chip has had, over months that actually hold reading. **(5) Shared reading recaps** — `kitabi.in/reader/<handle>/recap/<key>`, a public, server-rendered page carrying the window's numbers, its calendar and **the books finished in it**, each linking into the catalogue. The link is *derived* from the handle and the window (`recapKeyFor` ↔ `recap_service.parse_recap_key`, one fixture table in both test suites), so the app prints it on the card with no network at all — which makes it guessable, so the protection is a gate: `profiles.recaps_visible`, off by default, its own flag beside `profile_visible` and `library_visible` (migration `000052`, which also adds `utc_offset_minutes` so the server cuts a window on the reader's calendar days rather than UTC's). Every window has a link; the page is `noindex` **and** robots-disallowed, because `<key>` is an infinite key space and noindex stops indexing, not fetching (7 Sep). Tests: `test_recap_key.py` + `recap_key_test.dart` (the shared table), `test_public_recap.py` (the gate, the late-night sitting, the `finish_date ?? updated_at` rule), `recap_link_sheet_test.dart`, `month_card_calendar_test.dart`, `share_today_door_test.dart`, and 10 new renderer assertions in `landing-page/tests/cases.js`. **Amended the same day, on the first real share:** the link was opt-in behind a button, so the first card the owner sent carried the caption but no link and the recipient had nowhere to go. **Sharing is the consent now** — `ShareSheetScaffold.onBeforeShare` publishes before the capture, so the link is on that very image and in its caption, and the sheet prints the real URL plus "Sharing adds this link… turn it off any time in Profile" *before* Share is tapped rather than as a snackbar after. `recaps_visible` keeps its false default and its Profile switch: a reader who never shares publishes nothing. A publish that fails offline strips the link rather than sending one that 404s.
- **7 Sep 2026** — **Supabase egress over quota: measured, then bounded at the origin — no upgrade.** The organisation went over its free-tier egress (5.75 GB of 5 GB; grace period to 7 Oct 2026, then the Fair Use Policy and 402s). Not upgrading: Pro is $25/mo per org, and [docs/environments.md](docs/environments.md) §11 records the real trigger as the catalog outgrowing 500 MB, which at 30 MB it is nowhere near. **The driver was not what it looked like.** The obvious suspects were the nightly `pg_dump` and the covers bucket; `pg_stat_statements` on production said otherwise. The dump moves ~3 MB of table data (the 30 MB database is indexes) — under 100 MB a month, so `backup.yml` keeps its nightly cadence. Storage's own object lookups number ~17,000 since July, so the bucket cannot have produced 5 GB either. The `postgres` role — the API — made 5 million calls returning 33 million rows, and the chart's steady 150–200 MB a day (one 599 MB spike on 17 Aug, an ETL day) is **database egress from rendering the public site**: every render of home, browse and the hubs ran the three facet counts (language/form/genre, ~239,000 calls each since July, ~3,700 a day) plus the page queries, and browse is combinatorial — `noindex, follow` does not stop a crawler fetching every filter combination to read the tag, then following the chips into the next. The edge cache is per Cloudflare data centre and evicts freely on the free tier, so it bounded nothing at the origin. A second, smaller stream: the hourly `merge_exact` job loaded every author (~700 B a row of bio, image URL and search columns) and publisher, ~25 MB a day for a job that reads four columns. **Four changes, none a bill.** (1) `app/core/ttl_cache.py`, an in-process TTL cache mirroring the console's, and `public_service` memoises the facet counts and the whole home/browse/hub payloads for ten minutes — one computation per distinct key per TTL however many bots and data centres ask; `None` is never stored, the store is bounded, and a `# SCALE:` note covers multi-instance. (2) `merge_service.find_candidates` loads only `id, name, name_fold, created_at` as plain rows — not `load_only`, whose partial instances would sit in the identity map and trip `MissingGreenlet` in the `merge()` that follows, which is exactly what the first cut did. (3) `robots.txt` closes `/browse?` and `/search?` — bare `/browse`, `/search` and every hub stay open. (4) The app fetches every remote image through `kitabi.in/img/c` (`core/widgets/image_proxy.dart`, applied inside `netImage` and the promo art, display-only) — a smaller lever than first thought, but each fresh install, emulator and `adb uninstall` still pulled the catalogue's covers from the metered bucket; verified against production that repeat fetches carry an `age` header and never reach Supabase. Left alone on purpose: `pool_recycle=280` (~5,700 pooler re-auths a day, maybe 20 MB of TLS handshakes) — worth a look if the next cycle is still tight, but Supavisor's idle timeout is the constraint the comment there names. Not verifiable from here: who the ~3,700 daily renders are — Railway logs need `railway login`. Tests: `test_public_cache.py` (facets served from memory, browse keyed on every filter, a missing hub never pinned, single-flight under concurrency, bounded store, the narrow merge load still finds exact duplicates) and `image_proxy_test.dart`; the conftest clears the cache around every test.
- **5 Sep 2026** — **The approval queue reaches printings.** `PATCH
  /catalog/works/{id}` has been wiki-moderated since 8 Jul 2026 — a change to a
  book somebody else contributed is queued for them, and the app says so.
  `PATCH /catalog/editions/{id}`, the line beside it, had no gate at all: it
  called `update_edition` directly, so any signed-in reader could rewrite any
  printing's **ISBN, page count, format, publisher and cover URLs** on the
  shared catalogue, live, with no approval and no revision record. That is the
  more destructive half — a wrong page count is a lie against every progress
  figure, a wrong ISBN sends the next scanner to the wrong book — and it is
  what made the add-book duplicate fork's "improve this entry" path write a
  reader's captured cover onto a stranger's edition.

  Both endpoints now answer the same shape (`{applied, revision_id, …}`) and
  share one queue: `work_revisions.edition_id` (migration `000050`, nullable —
  null still means a Work edit) rather than a second table, because an edition
  edit is still an edit to the book, its approver is still the Work's
  contributor (an Edition carries no `created_by_user_id`), and the inbox, the
  admin escalation and `decide_revision` were all already written against that
  row. One predicate, `_may_edit`, now answers "whose row is this?" for the
  work edit, the edition edit and the reader-facing merge.

  App side: the edit form reads `applied` on both halves and shows
  `editPendingApproval`; the book page's cover actions say "sent for review"
  instead of "Cover updated" and stop mirroring an unaccepted cover into the
  shelf cache; the Pending-edits inbox renders an edition revision as **"This
  printing · ISBN · format · pages"** and diffs it against that printing,
  matched by id (never `editions[0]`). A total typed into the reading timer
  still lands in the device-local mirror whatever the catalogue decides — the
  reader's own progress never waits on someone else's approval.

  A genuinely wrong ISBN stays correctable: it queues rather than being
  refused. An ISBN that already names *another* book is still an immediate 409,
  so the app can go on offering that book instead of promising an approval that
  could never apply.
