...
| Mobile app (Android) | — | Play Store internal testing | Release **AAB build 26** (`app/build/app/outputs/bundle/release/app-release.aab`) via `scripts/build_aab.sh`, uploaded to Play Console internal testing. Google-managed app signing (upload key local at `~/keys/kitabi-upload.jks`, gitignored). R8 minification off (was stripping WorkManager/Firebase registrars). **⚠️ Upload key mismatch, open 31 Jul 2026:** Play rejects bundles signed with `~/keys/kitabi-upload.jks` (SHA1 `30:CE:13:EA…4C:1D:9F`) because this listing is enrolled with **rupee-diary's** upload key (SHA1 `5F:38:E0:15…61:DE:94:43`, at `~/keystores/rupeediary-upload.jks`) — build 26 must have gone up signed with it. Resolution chosen: keep Kitabi's own key and request an **upload key reset** in Play Console (App integrity → App signing), submitting `~/keys/kitabi-upload-certificate.pem`. Play App Signing is enabled, so the app-signing key devices verify is unaffected, and `assetlinks.json` already carries this cert's SHA-256 (`0E:BA:38…`) so app links need no change. **No Android upload is possible until Google actions the reset (~1–2 days);** the build-107 AAB is already correctly signed and can go up as-is once it lands |
