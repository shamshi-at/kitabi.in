...
- **Railway dashboard env vars** (production) — `DATABASE_URL` (Supavisor pooler), `SUPABASE_URL`,
  `ENV=production`, `SCHEDULER_ENABLED=true`. Optional, not yet set: `AMAZON_ASSOCIATE_TAG` /
  `FLIPKART_AFFILIATE_ID` (buy links render untagged until these exist — set after the
  Associates account is approved; plain URL params, not credentials).
...
- **8 Aug 2026** — **Every book page now sends buyers somewhere — and can pay the bills.**
  ...
  the rendered rail verified visually against the real edge template. Owner actions open:
  Amazon Associates India signup, then the Railway vars.
